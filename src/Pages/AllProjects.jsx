...
const AllProjects = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <Tag className='px-4 py-2 rounded-5 font12px fw-bold' color={color}>{msg}</Tag>
        );
      }
    },
    {
      ...
      width: 130,
      render: (record) => 
        <a className=''> <EditOutlined style={{ marginRight: '8px',color:'blue'}}  onClick={() => editForm(record)}/> </a>
      ,
    },
  ];
  ...
  const allData = async () => {
    try {
      ...
      const response = await axios.post(`${get_all_propoposal_url}`,payload, API_HEADER); 
